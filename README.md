...
训练与评估（细化）
- SFT（LoRA）：见上文“一步跑通”。
- GRPO（LoRA）：`scripts/grpo.sh` 与 `scripts/grpo.ps1` 固定使用 `contract_holdings`+`mse_holdings` 的 0.4/0.6 联合奖励，外部插件路径固定为 `src/plugins/grpo/holdings_plugin.py`。
...
- Bash（Linux/macOS）：
  ...
  - 提示：
  - 脚本已写死 `contract_holdings`+`mse_holdings`（0.4/0.6）。如需调整比例，请直接修改 `scripts/grpo.sh` / `scripts/grpo.ps1` 中的常量；其中 `contract_holdings` 现会检查生成文本中是否含 `<think>…</think>`，缺失将记 0 分。
